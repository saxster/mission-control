...
function peekLatestOutput(
  cronDir: string,
  jobId: string,
): { lastRunAt: string | null; lastOutput: string | null; lastOutputKind: 'response' | 'silent' | 'log' | null } {
  const outputDir = join(cronDir, 'output', jobId)
  try {
    if (!existsSync(outputDir) || !statSync(outputDir).isDirectory()) {
      return { lastRunAt: null, lastOutput: null, lastOutputKind: null }
    }
    ...
    if (files.length === 0) return { lastRunAt: null, lastOutput: null, lastOutputKind: null }
...
function scanCronJobsForProfile(profile: HermesRuntimeProfile): HermesCronJob[] {
  ...
  try {
    ...
    return jobs.map((job: any) => {
      ...
      const { lastRunAt, lastOutput, lastOutputKind } = peekLatestOutput(cronDir, id)
