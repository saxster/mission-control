...
import { warmGatewaySessionCache } from './sessions';
  ...
  try {
    ...
    if (!webhookListenerInitialized) {
      ...
      if (!isBuildPhase && !isTestMode) {
        ...
        import('./local-session-index').then(({ queueLocalSessionIndexSync }) => {
          queueLocalSessionIndexSync()
        }).catch(() => {
          // Silent - background index warmup is optional
        })
      }

      if (!isBuildPhase) {
        try {
          warmGatewaySessionCache()
        } catch {
          // Silent - cache warmup is opportunistic
        }
      }
    }
