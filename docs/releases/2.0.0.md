...
- The old memory panel was redesigned into an Obsidian-style knowledge surface that is now exposed in the current product as the Knowledge Base.
- Mission Control 2.0.0 originally shipped with a filesystem memory browser, link/context endpoints, memory health signals, and an interactive graph view for linked knowledge.
- Those foundations now sit behind the unified Knowledge Base surface instead of a separate Memory Browser affordance.
...
- The memory experience was redesigned into a graph-first, Obsidian-style knowledge system that later evolved into the Knowledge Base.
- The original memory browser improvements around lazy directory hydration, overflow handling, and graph fitting now apply to the unified Knowledge Base surface.
- The underlying APIs expanded around graph, health, links, context, and processing actions before being consolidated under the Knowledge Base backend.
- Boot-time prefetching and panel persistence reduced the need to re-fetch heavy knowledge state on every navigation.
...
- Knowledge graph upgrades from Canvas to Reagraph WebGL
...
- Knowledge graph blank states, overflow, sizing, auto-fit, and speed issues
