...
Note for current operators: newer post-2.0.1 builds now expose the unified Knowledge Base as the canonical knowledge surface. The legacy `/api/memory/*` routes remain available as a temporary compatibility layer over `/api/knowledge-base/*`, but new integrations should target the Knowledge Base endpoints directly.

Current post-2.0.1 governance builds also add a first-class Knowledge Base governance queue. High-risk domains now use bounded live source verification when possible, explicit overrides remain durable and labeled, and retrieval/search de-prioritize unreviewed or overridden material instead of presenting it as equal evidence.
...
- In current post-2.0.1 builds, `/api/memory/*` should be treated as a deprecated compatibility layer over `/api/knowledge-base/*`
- Current compatibility builds now emit explicit deprecation/sunset headers on `/api/memory/*` responses and log legacy-route usage to support safe removal planning
